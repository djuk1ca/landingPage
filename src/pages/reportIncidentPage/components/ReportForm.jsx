...
export default function ReportForm(){
    ...
    return(
        <form className="w-4/5 mt-8 lg:mt-0 bg-[#1B1B1B] opacity-75 border-[1px] border-[#C4CB38] rounded-[50px] text-center px-14 py-5">
            ...
            <div className="w-full max-h-20 mt-8 bg-transparent text-[#C1C1C1] border border-white rounded-full py-3 pl-6 pr-8  leading-normal  focus:border-gray-300 flex justify-between items-center">
                <ul className='flex gap-3'>{files.length != 0 ? files.map((file, index) => (<li key={index}>{file.name} ({file.type})</li>)) : "Supporting Evidence (Attach files)"}</ul>
            ...
            <button className="mt-8 px-12 lg:px-32 py-1 bg-transparent border-[4px] border-[#C4CB38] text-lg tracking-[20px] text-[#C4CB38] font-black rounded-[10px]" type='submit'>SUBMIT</button>
